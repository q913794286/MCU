...
uint8_t cmd_flg=0;
/********************* UART1中断函数************************/
void UART1_init (void) interrupt UART1_VECTOR using 2
{
	char buf;
    if(RI)
	{
	    ...
	    {		 
			  buf = SBUF;
				uart1_set_TimeOut(20); 	//100ms未收到数据则接收结束
				...
				if(config_mode==1)
				{
				  RX1_Buffer[COM1.RX_Cnt++] = buf;
					switch(buf)
					{
						...
						case 0x81: if(cmd_flg == 1 && COM1.RX_Cnt == 1){printf("MU");uart1_cmd = 1 ;cmd_flg = 0; COM1.RX_Cnt = 0;} break; //进入配置模式

						case 0x82: cmd_flg = 2; COM1.RX_Cnt = 0; break;
						case 0x83: if(cmd_flg == 2 && COM1.RX_Cnt == 1){uart1_cmd = 2; cmd_flg = 0; COM1.RX_Cnt = 0;} break; //进入GPRS查询配置模式	

						case 0x84: cmd_flg = 3; COM1.RX_Cnt = 0; break;
						case 0x85: if(cmd_flg == 3 && COM1.RX_Cnt == 1){uart1_cmd = 3; cmd_flg = 0; COM1.RX_Cnt = 0;} break;	 //单片机串口配置
		
						case 0x86: cmd_flg = 4; COM1.RX_Cnt = 0; break;
						case 0x87: if(cmd_flg == 4 && COM1.RX_Cnt == 1){uart1_cmd = 4; cmd_flg = 0; COM1.RX_Cnt = 0;} break;	 //GPRS机配置
		
						case 0x88: cmd_flg = 5; COM1.RX_Cnt = 0; break;
						case 0x89: if(cmd_flg == 5 && COM1.RX_Cnt == 1){uart1_cmd = 5; cmd_flg = 0; COM1.RX_Cnt = 0;} break; //退出单片机配置模式
						default:  break;
					}				
				}
				 else{
...
void UART2_int (void) interrupt UART2_VECTOR using 3
{
	if(RI2)
	{
		...
		{
			uart2_set_TimeOut(20); //100ms未收到数据则接收结束
