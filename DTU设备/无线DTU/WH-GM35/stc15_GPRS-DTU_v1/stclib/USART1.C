	    ...
	    {		 
		   static	uint8_t cmd_flg=0;
		   	buf	= SBUF;
						...
						if(uart1_cmd == 0)
			{	
				uart1_set_TimeOut(20); 	//200ms未收到数据则接收结束
			}
			...
			if(config_mode==0)
			{
				switch(buf)
				{
					...
					case '+': if(cmd_flg == 2 && COM1.RX_Cnt == 1){uart1_cmd = 3; cmd_flg = 0; COM1.RX_Cnt = 0;} break;	 //单片机串口配置
	
					case 'G': cmd_flg = 3; COM1.RX_Cnt = 0; break;
					case '*': if(cmd_flg == 3 && COM1.RX_Cnt == 1){uart1_cmd = 4; cmd_flg = 0; COM1.RX_Cnt = 0;} break;	 //GPRS机配置
	
					case 'Q': cmd_flg = 4; COM1.RX_Cnt = 0; break;
					case '!': if(cmd_flg == 4 && COM1.RX_Cnt == 1){uart1_cmd = 5; cmd_flg = 0; COM1.RX_Cnt = 0;} break; //退出单片机配置模式
					default:  break;
				}				
			 }
