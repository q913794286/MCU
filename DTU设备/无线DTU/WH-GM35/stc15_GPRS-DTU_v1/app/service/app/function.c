...
void function(void)
{
	...
	while(1)
	{
	  ...
	  *  uart1_cmd = 5 退出配置模式  
	  */
	  if(aes128.state==2)	//加密模式
	  {
	 	uart1_check_TimeOut();  //串口超时检测
	  	uart2_check_TimeOut(); 
		}
	  if(aes128.state==1)	 //透传模式
	  {
	  	 usart1_and_usart2();
	  }		
...
//	EEPROM_write_n(0x00,&RX1_Buffer[0],COM1.RX_Cnt);	  //uart1_cnt = 48Byte
	delay_ms(6);
	Printf1("ok",2);
...
void gprs_conf()
{
	...
	delay_ms(6);		//擦除时间4ms~6ms  
	EEPROM_write_n(513,&RX1_Buffer[0],COM1.RX_Cnt);	  //uart1_cnt = 48Byte
	delay_ms(6);
	Printf1("ok",2);
}
sbit LED1 = P1^1;
