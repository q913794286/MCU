...
gpio_init_led(void)
{
	...
	PIN_FUNC_SELECT(PERIPHS_IO_MUX_MTCK_U, FUNC_GPIO13);  //设置输出模式    Red
	gpio_output_set(0, BIT13, BIT13, 0);      //设置输出低电平
	//gpio_output_set(BIT13, 0, BIT13, 0);      //设置输出高电平
	//GPIO_OUTPUT_SET(13,1);

	PIN_FUNC_SELECT(PERIPHS_IO_MUX_MTDI_U, FUNC_GPIO12);  //设置输出模式 green
	gpio_output_set(0, BIT12, BIT12, 0);      //设置输出低电平
	//gpio_output_set(BIT12, 0, BIT12, 0);      //设置输出高电平
	//GPIO_OUTPUT_SET(12,1);

	PIN_FUNC_SELECT(PERIPHS_IO_MUX_MTMS_U, FUNC_GPIO14);  //设置输出模式 blue
	gpio_output_set(0, BIT14, BIT14, 0);      //设置输出低电平
	//gpio_output_set(BIT14, 0, BIT14, 0);      //设置输出高电平
...
user_gpio_timer_init(void) {
	...
	static  os_timer_t gpio_timer;  // 管脚功能选择
