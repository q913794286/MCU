...
#line 1 "app\service\GPRS\GPRS.c" /0
...
#line 2 "app\service\GPRS\GPRS.c" /0
...
#line 3 "app\service\GPRS\GPRS.c" /0
...
#line 4 "app\service\GPRS\GPRS.c" /0
...
#line 5 "app\service\GPRS\GPRS.c" /0
...
#line 6 "app\service\GPRS\GPRS.c" /0
...
#line 7 "app\service\GPRS\GPRS.c" /0
...
#line 8 "app\service\GPRS\GPRS.c" /0
...
#line 9 "app\service\GPRS\GPRS.c" /0
...
#line 10 "app\service\GPRS\GPRS.c" /0
 
 
 uint8_t code *AT   ="AT";
 uint8_t code *GMM  ="AT+GMM";   		 
 uint8_t code *CGSN ="AT+CGSN";  		 
 uint8_t code *ATF  ="AT&F";     		 
 uint8_t code *ATW  ="AT&W";     		 
 uint8_t code *CFUN ="AT+CFUN=1"; 		 
 uint8_t code *ICF  ="AT+ICF=3,0";        
 uint8_t code *IPR  ="AT+IPR=115200";     
 uint8_t code *CLCK ="AT+CLCK=SC,2";          
 
 uint8_t code *CPIN ="AT+CPIN?";
 
 uint8_t code *COPS ="AT+COPS?";          
 uint8_t code *CREG1 ="AT+CREG=1";         
 uint8_t code *CREG2 ="AT+CREG?";			 
 
 uint8_t code *CSQ  ="AT+CSQ";            
 uint8_t code *CGREG="AT+CGREG?";
 uint8_t code *CGATT="AT+CGATT=1"; 		 
 uint8_t code *CCSTT ="AT+CSTT=\"CMNET\"";	 		 
 uint8_t code *UCSTT ="AT+CSTT=\"3gnet\"";
 uint8_t code *CGAUTO="AT+CGAUTO=1";     
 uint8_t code *CIFSR="AT+CIFSR";         
 ...
 uint8_t code *CIPSHUT  = "AT+CIPSHUT";   
 uint8_t code *CIICR    = "AT+CIICR";   
 uint8_t code *CIPSTATUS = "AT+CIPSTATUS";   
 
 uint8_t code *CDNSGIP = "AT+CDNSGIP=";	  
 uint8_t code *DNSserver="8.8.8.8";		 
 uint8_t code *CIPMODE   ="AT+CIPMODE=1";  
 ...
 void AT_Send(uint8_t *ATX)                                      
 {    
 (void)memset(RX2_Buffer,0,128);	 
 COM2.RX_Cnt=0;                                          
 send_string(ATX);   					  
 send_string("\r");  
 delay_ms(500);                                  
 ...
 res = gprs_cmd(CIPMODE,RX2_Buffer,"OK",500);	 
 if(res==1)
 printf("%s%s%s","入透传模式:",RX2_Buffer,"\n");
 else
 printf("%s","进入透传失败！");
 
 if(!strstr(RX2_Buffer,"46000"))		 
 {	
 ...
 res = gprs_cmd(CIICR,RX2_Buffer,"OK",500);   
 if(res==1)
 printf("%s%s%s","激活移动场景:",RX2_Buffer,"\n");
 else
 printf("%s","激活移动场景失败！");						 	
 
 res = gprs_cmd(CIFSR,RX2_Buffer,"OK",500);		 
 ...
 printf("server=%s\r\n",addr);
 gprs_status = gprs_cmd(addr, RX2_Buffer, "CONNECT", 1000);		    
 if(gprs_status==1)
